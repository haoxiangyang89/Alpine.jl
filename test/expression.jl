@testset "Expression Parsing || bilinear || Affine || exprs.jl" begin
    ...
    test_solver = PODSolver(nlp_solver=IpoptSolver(print_level=0),mip_solver=CbcSolver(log_level=0),log_level=100)
...
@testset "Expression Parsing || part1 " begin
    m = Model(solver=PODSolver(nlp_solver=IpoptSolver(),mip_solver=CbcSolver(log_level=0),log_level=100))
    @variable(m, x[1:4]>=0)
