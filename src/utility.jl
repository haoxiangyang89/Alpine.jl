...
function update_opt_gap(m::PODNonlinearModel)
    ...
    if m.best_obj in [Inf, -Inf]
        ...
        p = convert(Int, round(abs(log(10,m.rel_gap))))
...
function update_boundstop_options(m::PODNonlinearModel)
    ...
    if m.mip_solver_id == "Gurobi"
        ...
        if m.sense_orig == :Min
            m.rel_gap_ref == "ub" ? stopbound=(1-m.rel_gap+m.tol)*abs(m.best_obj) : stopbound=(1-m.rel_gap+m.tol)*abs(m.best_bound)
...
Check if the solution is alwasy the same within the last disc_consecutive_forbid_iter iterations. Return true if suolution in invariant.
"""
function check_solution_history(m::PODNonlinearModel, ind::Int)

    m.disc_consecutive_forbid_iter == 0 && return false
    (m.logs[:n_iter] < m.disc_consecutive_forbid_iter) && return false

    sol_val = m.bound_sol_history[mod(m.logs[:n_iter]-1, m.disc_consecutive_forbid_iter)+1][ind]
    for i in 1:(m.disc_consecutive_forbid_iter-1)
        search_pos = mod(m.logs[:n_iter]-1-i, m.disc_consecutive_forbid_iter)+1
