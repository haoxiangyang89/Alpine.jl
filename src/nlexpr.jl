...
function expr_parsing(m::PODNonlinearModel)
	...
	(m.log_level > 199) && println("[OBJ] $(m.obj_expr_orig)")
	...
	for i in 1:m.num_constr_orig
		...
		(m.log_level > 199) && println("[CONSTR] $(m.constr_expr_orig[i])")
	end
...
function expr_conversion(m::PODNonlinearModel)
	...
	m.log_level > 199 && println("type :: ", m.obj_structure)
	...
	for i in 1:m.num_constr_orig
		...
		m.log_level > 199 && println("type :: ", m.constr_structure[i])
...
function expr_constr_parsing(expr, m::PODNonlinearModel, idx::Int=0)
    ...
    for i in 1:length(m.constr_pattern_methods)
        is_strucural = eval(m.constr_pattern_methods[i])(expr, m, idx)
        return
    end

    # Recognize built-in special structural pattern
    if m.convexity_recognition
